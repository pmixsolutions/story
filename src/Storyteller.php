...
class Storyteller
{
     ...
     *
     * @param  string  $type
     * @param  \Pmixsolutions\Story\Model\Content|null  $content
     *
     * @return string
     */
    public function url($type, $content = null)
    {
        ...
        if (in_array($type, $predefined)) {
            return handles("pmixsolutions/story::{$type}");
        }
    ...
    {
        $format = $this->app['config']->get("pmixsolutions/story::permalink.{$type}");

        if (is_null($format) || ! ($content instanceof Content)) {
            return handles('pmixsolutions/story::/');
        }
        ...
        return handles("pmixsolutions/story::{$format}");
    }
}
